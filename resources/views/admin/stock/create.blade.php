        ...
        <div class="form-container">
            ...
            <form method="POST" action="{{ route('admin.stock.store') }}" id="stockForm">
                ...
                <div class="form-group">
                    ...
                    <div class="type-selection">
                        <div class="type-option">
                            <input type="radio" name="type" value="in" id="type_in" {{ old('type') == 'in' ? 'checked' : '' }}>
                        ...
                        <div class="type-option">
                            <input type="radio" name="type" value="out" id="type_out" {{ old('type') == 'out' ? 'checked' : '' }}>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <div class="help-text">Référence unique générée automatiquement</div>
                        </div>
                    </div>
                ...
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-select" id="warehouse_id" name="warehouse_id" required>
                                ...
                                @foreach($entrepots as $entrepot)
                                    <option value="{{ $entrepot->id }}" {{ old('warehouse_id') == $entrepot->id ? 'selected' : '' }}>
                                        {{ $entrepot->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="product_input" class="form-label">Produit/Stock <span class="required">*</span></label>
                            <input type="text" 
                                   class="form-control" 
                                   id="product_input" 
                                   name="product_name" 
                            ...
                            <datalist id="products_list">
                                @foreach($stocks as $stock)
                                    <option value="{{ $stock->item_name }}" data-stock-id="{{ $stock->id }}" data-quantity="{{ $stock->quantity }}">
                                        {{ $stock->item_name }} (Stock: {{ $stock->quantity }})
                                    </option>
                                @endforeach
                            </datalist>
                            <input type="hidden" id="stock_id" name="stock_id" value="{{ old('stock_id') }}">
                            <div class="help-text">Saisir le nom du produit ou sélectionner un produit existant</div>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Générer la référence automatiquement au chargement
    generateReference();

    // Régénérer la référence quand le type change
    document.querySelectorAll('input[name="type"]').forEach(function(radio) {
        radio.addEventListener('change', generateReference);
    });

    // Gestion de l'auto-complétion du produit
    ...
    productInput.addEventListener('input', function() {
        ...
        const options = document.querySelectorAll('#products_list option');
        let found = false;
        
        options.forEach(function(option) {
            if (option.value === selectedProduct) {
                stockIdInput.value = option.getAttribute('data-stock-id') || '';
                found = true;
            }
        });
    ...
    document.getElementById('stockForm').addEventListener('submit', function(e) {
        ...
        if (!productName) {
            e.preventDefault();
            showToast('Veuillez saisir ou sélectionner un produit', 'error');
            return;
        }
    });
});
...
function generateReference() {
    const type = document.querySelector('input[name="type"]:checked');
    if (type) {
        // Appeler le serveur pour générer une référence unique
        fetch('{{ route("admin.stock.generate-reference") }}', {
            method: 'POST',
            headers: {
        ...
        .catch(error => {
            ...
            // Fallback avec génération côté client
            const now = new Date();
            const year = now.getFullYear();
            const prefix = type.value === 'in' ? 'STK-IN' : 'STK-OUT';
            const random = Math.floor(Math.random() * 9999) + 1;
            document.getElementById('reference').value = `${prefix}-${year}-${random.toString().padStart(4, '0')}`;
        });
    }
}
