<?php
/**
 * 🧹 NETTOYAGE FINAL CORRECT CSAR
 * 
 * Ce script supprime définitivement toutes les données fictives
 * en utilisant les bonnes colonnes (nom, prenom pour demandes)
 */
...
try {
    ...
    echo "🧹 NETTOYAGE FINAL CORRECT CSAR\n";
    echo "==============================\n\n";
    ...
    // 2️⃣ NETTOYAGE DE LA TABLE DEMANDES (avec nom/prenom)
    echo "📋 2. NETTOYAGE DE LA TABLE DEMANDES\n";
    echo "===================================\n";

    // Supprimer toutes les demandes fictives de la table demandes
    $stmt = $pdo->query("DELETE FROM demandes WHERE nom IN (
        'Mariama', 'Amadou', 'Fatou', 'Ibrahima', 'Aïcha', 'Jean', 'Marie', 'Aminata', 'Moussa', 'Khadija'
    ) OR prenom IN (
        'Diop', 'Ba', 'Sarr', 'Fall', 'Ndiaye', 'Dupont', 'Martin', 'Diallo', 'Traoré', 'Sow'
    )");
    ...
    // 3️⃣ NETTOYAGE DE LA TABLE PUBLIC_REQUESTS (avec full_name)
...
try {
    ...
    file_put_contents('verification_finale_correcte.php', $verificationScript);
    echo "✅ Script de vérification créé : verification_finale_correcte.php\n\n";
    ...
    echo "   1. Exécuter : php verification_finale_correcte.php\n";
